...
>(
  (
    ...
    const toggleSidebar = React.useCallback(() => {
      console.debug("[Sidebar] toggleSidebar clicked", { isMobile })
      return isMobile
        ? setOpenMobile((open) => {
            const next = !open
            console.debug("[Sidebar] setOpenMobile", { open: next })
            return next
          })
        : setOpen((open) => {
            const next = !open
            console.debug("[Sidebar] setOpen (desktop)", { open: next })
            return next
          })
    }, [isMobile, setOpen, setOpenMobile])

    // Keyboard shortcut removed for simplicity
      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            id="mobile-sidebar"
            className="w-48 bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] z-[9999]"
            side={side}
          >
